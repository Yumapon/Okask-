...
public class DefaultDAOManager extends AbstructDAOManager implements DAOManager {
	...
	public void taskstorageDAO(Date deadLine, String taskName, String content, String client) {
		try {
			//task格納用のSQLを用意
			pre_statement = conn.prepareStatement(stragetasksql, ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_READ_ONLY);
			pre_statement.setDate(1, deadLine);
			pre_statement.setString(2, taskName);
			pre_statement.setString(3, content);
			pre_statement.setString(4, client);
